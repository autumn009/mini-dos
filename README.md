...
　MS-DOSで動作する。AZA用のZ80ソースコードをMASM用の8086ソースコードに変換する。yaccとCで書かれている。一部の命令はINCファイルの定義に依存しているのでこれをINCLUDEして使う。全ての命令を返還できるわけではなく、変換できない時はその旨をコメントに埋め込む。
